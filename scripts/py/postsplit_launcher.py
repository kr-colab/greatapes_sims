...
for i in range(len(params)):
    ...
    for j in range(reps):
        ...
        values = [str(params.loc[i,]["real_pop_size1"]),str(params.loc[i,]["real_pop_size2"]), burnin_path+burnin_prefix+"_RAND_"+burnin_rand+".trees", str(int(params.loc[i,]["real_gens"])), "0","1.5e-8","200000000",rand]
        write_sim_sh(var_names, values, prefix, out_path ,script_path, meta_path, rand, mutate, prefix_mut, time="120:00:00", mem = "128G")
        if (slurm):
            cmd = "sbatch "+rand+".sh"
        else:
            cmd = "sh "+rand+".sh"
