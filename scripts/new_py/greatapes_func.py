...
def callset_to_masked_gt(callset, c, acc):
    ...
    pos = pos[acc[pos-1]]
    return(allel.GenotypeArray(gt), pos, alt, call_shape[1], vcf_features)

def merge_allel_arrays(pos1, pos2, ac1, ac2, n1, n2, alt1=None, alt2=None):
    """
    ...
    merged_alt1 = np.full(len(p), 'N', dtype=str)
    merged_alt2 = merged_alt1.copy()
    if (alt1 is not None) and (alt2 is not None):
        merged_alt1[is_pos1] = alt1
        merged_alt2[is_pos2] = alt2
    is_nottri = np.logical_not((merged_alt1 != merged_alt2) * (merged_alt1 != 'N') * (merged_alt2 != 'N'))
    ...
    return (p[is_nottri], merged_ac1[is_nottri], merged_ac2[is_nottri])
