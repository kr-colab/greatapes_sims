...
#infilepath = paste0("../../output/joined_stats/sup-rand-id_", neutsup,"_rep_0_win-size_1000000_sample-size_10.npz")
...
parser$add_argument('outfilepath',  type="character", help="Path to out .tsv file")


infilepath = parser$parse_args()$npzfilepath
outfilepath = parser$parse_args()$outfilepath


colorder=c("value", "stat", "spp1", "spp2", "chr", "start", "end", "n_acc_bases")

win_size = as.integer(strsplit(infilepath, "_")[[1]][7])
## TODO: add the correct extension and edit the wreite.table below!!

npz = np$load(infilepath)
npz$files
dxy_mat = npz$f[["dxy"]]
...
coords = data.table(rn = names(coord_starts), chr=chroms, start=coord_starts, end = coord_starts+win_size) #idk why I didn't use coord_ends?
...
setcolorder(dxy, colorder)
write.table(dxy, outfilepath, sep="\t", quote=FALSE, row.names=FALSE)