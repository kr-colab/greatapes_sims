...
parser.add_argument('seed', type=int, default=8991)
parser.add_argument('recapN', type=int, default=10000)
...
tsu.dump(f"{trees_path}{rand_id}_rep{rep}.union.trees")
...
mut_map = msp_mutation_rate_map(exons, total_mut_rate, region_mut_rate, int(recap_tsu.sequence_length))
...
# saving to output
np.savez(f"{trees_path}rand-id_{rand_id}_rep_{rep}_win-size_{win_size}_sample-size_{sample_size}.npz", windows, dxy, labels)
